...
def get_objects_detection_api_call_data_of(source,
    ...
    if specific_object == OptionalObjectDetection.Vehicles:
        ...
        print(mainly_data_dict_response)
...
def get_object_info_and_coordinates_by_terms(source,
    ...
    for itm in all_data_list:
        ...
        if min_plate_detection_score is not None:
            if itm["plate"]["score"] < min_plate_detection_score:
                continue
        if min_plate_text_reading_score is not None:
            if itm["dscore"] < min_plate_text_reading_score:
                continue
        if color is not None:
            if itm["vehicle"]["color"]["value"] == color:
                continue
        if vehicle_orientation is not None:
            if itm["vehicle"]["prop"]["orientation"] == vehicle_orientation:
                continue
        if vehicle_company is not None:
            if itm["vehicle"]["prop"]["make_model"]["make"] == vehicle_company:
                continue
        if vehicle_model is not None:
            if itm["vehicle"]["prop"]["make_model"]["model"] == vehicle_model:
                continue
        if vehicle_plate_number is not None:
            if itm["plate"]["props"]["plate"]["value"] == vehicle_plate_number:
                continue
        out_info = {}
        try:
            # Building the info
